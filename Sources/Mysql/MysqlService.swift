...
public class MysqlService {
    ...
    public class func listDB(_ dbNameWildcard:String? = nil) -> [String]? {
    ...
    public class func listTable(_ dbName:String, tableNameWildcard:String? = nil) -> [String]? {
        guard let mysql = connServer() else {
            return nil
        }
    ...
    public class func query(dbName:String,sql:String,callback:(_ status:Bool, _ result:Array<Array<String?>>?)->()){
        let mysql = MySQL()
        guard mysql.connect(host: MysqlService.defaultHost, user: MysqlService.defaultUser, password: MysqlService.defaultPassword, db: MysqlService.defaultDB, port: UInt32(MysqlService.defaultPort)) else {
